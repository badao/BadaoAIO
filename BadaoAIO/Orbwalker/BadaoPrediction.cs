...
namespace BadaoAIO.Orbwalker
{
    public static class BadaoPrediction
    {
            ...
            public CollisionableObjects[] CollisionObjects =
        {
            ...
            public Vector3 From
            {
                get { return _from.ToVector2().IsValid() ? _from : ObjectManager.Player.ServerPosition; }
                set { _from = value; }
            }
            ...
            public Vector3 RangeCheckFrom
            {
                get
                {
                    return _rangeCheckFrom.ToVector2().IsValid()
                        ? _rangeCheckFrom
                        : (From.ToVector2().IsValid() ? From : ObjectManager.Player.ServerPosition);
                }
        ...
        public class PredictionOutput
        {
            ...
            {
                get { return _unitPosition.ToVector2().IsValid() ? _unitPosition.SetZ() : Input.Unit.ServerPosition; }
            ...
            List<Vector2> path, float speed = -1)
        {
            ...
            // set standar output
            Vector2 castpos = target.ServerPosition.ToVector2();
            Vector2 unitpos = target.ServerPosition.ToVector2();
            ...
            if (path.Count <= 1)
            {
                ...
                castpos = target.ServerPosition.ToVector2();
                unitpos = target.ServerPosition.ToVector2();
                // target in range
                if (spell.RangeCheckFrom.ToVector2().Distance(castpos) <= spell.Range)
                    hitchance = HitChance.High;
                // target out of range
                else
                {
                    ...
                    if (spell.Type == SkillshotType.SkillshotCircle)
                    {
                        ...
                        if (spell.RangeCheckFrom.ToVector2().Distance(castpos) <=
                            spell.Range + spell.Width + target.BoundingRadius - 40)
                        {
                            castpos = spell.RangeCheckFrom.ToVector2().Extend(castpos, spell.Range);
                ...
                return new PredictionOutput()
                {
                    UnitPosition = unitpos.ToVector3(),
                    CastPosition = castpos.ToVector3(),
                    Hitchance = hitchance
                };
            }
            ...
            if (Math.Abs(spell.Speed - float.MaxValue) < float.Epsilon && path.Count >= 2)
            {
                ...
                if (spell.Type == SkillshotType.SkillshotCircle)
                {
                    ...
                    var pos03 = pos02.Extend(spell.From.ToVector2(), distance03);
                    // lines length
                    ...
                    for (int i = 0; i <= 10; i++)
                    {
                        if (poses[i].Distance(spell.RangeCheckFrom.ToVector2()) <= spell.Range &&
                            poses[i].Distance(a) <= distance)
                        {
                            ...
                            {
                                UnitPosition = unitpos.ToVector3(),
                                CastPosition = poses[i].ToVector3(),
                                Hitchance = hitchance
                            };
                        }
                    }
                ...
                else
                {
                    ...
                    var distance01 = x;
                    var pos01 = a.Extend(b, distance01);
                    var range01 = spell.RangeCheckFrom.ToVector2().Distance(pos01);
                    // set standar position
                    unitpos = pos01;
                    castpos = pos01;
                    // hitchance high
            ...
            if (Math.Abs(spell.Speed - float.MaxValue) > float.Epsilon)
            {
                ...
                var castpos03 = unitpos03.IsValid()
                    ? spell.From.ToVector2().Extend(unitpos03, spell.From.ToVector2().Distance(unitpos03) - rad)
                    : new Vector2();
                if (castpos01.IsValid() && castpos02.IsValid() && castpos03.IsValid())
                {
                    ...
                    var Acosb =
                        Math.Acos(
                            Math.Abs(float.IsNaN(math.CosB(spell.From.ToVector2(), a, b))
                                ? 0.99f
                                : Math.Abs(math.CosB(spell.From.ToVector2(), a, b)))) * (180 / Math.PI);
                        ...
                        (spell.Type == SkillshotType.SkillshotLine && Acosb <= 110 && Acosb >= 70))
                    {
                        ...
                        for (int i = 0; i <= 10; i++)
                        {
                                ...
                                poses[i].Distance(a) <= distance)
                            {
                                ...
                                return new PredictionOutput
                                {
                                    UnitPosition = unitpos02.ToVector3(),
                                    CastPosition = poses[i].ToVector3(),
                                    Hitchance = hitchance
                                };
                            }
                        ...
                        {
                            UnitPosition = unitpos02.ToVector3(),
                            CastPosition = castpos02.ToVector3(),
                            Hitchance = HitChance.OutOfRange
                        };
                    }
                    // skillshot line + cone
                    else
                    {
                        ...
                        var range01 = spell.RangeCheckFrom.ToVector2().Distance(castpos02);
                        // hitchance high
                ...
                foreach (var objectType in objects)
                {
                    switch (objectType)
                    {
                                                ...
                                                spell.RangeCheckFrom)))
                            {
                                ...
                                if (
                                    minionPrediction.UnitPosition.ToVector2()
                                        .Distance(spell.From.ToVector2(), position.ToVector2(), true, true) <=
                        ...
                        case CollisionableObjects.Walls:
                            ...
                            for (var i = 0; i < 20; i++)
                            {
                                var p = spell.From.ToVector2().Extend(position.ToVector2(), step * i);
                        ...
                        case CollisionableObjects.YasuoWall:
                            ...
                            if (wallStart.Intersection(wallEnd, position.ToVector2(), spell.From.ToVector2()).Intersects)
                            {
                                var t = Variables.TickCount +
                                        (wallStart.Intersection(wallEnd, position.ToVector2(), spell.From.ToVector2())
